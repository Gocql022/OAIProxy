...
async function showOpenAIDeviceCode(verificationUri: string, userCode: string, expiresInMs: number): Promise<void> {
	...
	const action = await vscode.window.showInformationMessage(
		vscode.l10n.t(
			"OpenAI OAuth device sign-in\n\nDevice code: {0}\n\nSign-in page: {1}\n\nThe code expires in about {2} minutes.",
			userCode,
			verificationUri,
			String(expiresMinutes)
		),
		{ modal: true },
		vscode.l10n.t("Copy device code"),
		vscode.l10n.t("Open sign-in page")
	);
	if (action === vscode.l10n.t("Copy device code")) {
		await vscode.env.clipboard.writeText(userCode);
		void vscode.window.showInformationMessage(vscode.l10n.t("OpenAI device code copied to the clipboard."));
	} else if (action === vscode.l10n.t("Open sign-in page")) {
		try {
			await vscode.env.openExternal(vscode.Uri.parse(verificationUri));
		} catch {
			void vscode.window.showErrorMessage(
				vscode.l10n.t("Open {0} manually and enter code {1}.", verificationUri, userCode)
			);
		}
	}
}
		...
		if (!apiKey) {
			vscode.window.showErrorMessage(
				isXaiOAuth
						...
						: isOpenAICodexOAuth
							? vscode.l10n.t("Sign in to OpenAI / Codex with OAuth before checking quota.")
