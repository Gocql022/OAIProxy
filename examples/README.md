...
Normal provider API keys are stored by the extension as `oaicopilot.apiKey.<provider>`, not inside these snippets. TokenRouter keys use `oaicopilot.apiKey.tokenrouter`; its usage row links to the TokenRouter dashboard because no public key-scoped usage endpoint is documented. Fireworks usage checks reuse the normal provider key. For OpenAI and Anthropic usage/cost checks, enter the separate admin key in the configuration UI's `Usage Key` field; it is stored separately as `oaicopilot.usageApiKey.<provider>`.
